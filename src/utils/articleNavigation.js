...
export function buildArticleDetailParams(item, extra = {}) {
...
/** Active bottom-tab name (Home, Search, Chat, Notifications, Profile). */

export function getCurrentMainTab(navigation) {
  ...
  try {
    ...
    if (mainTabsRoute?.state?.routes?.length) {

      const idx = mainTabsRoute.state.index ?? 0;

      return mainTabsRoute.state.routes[idx]?.name || 'Home';
 ...
 * Push ArticleDetail onto MainAppStack so swipe/hardware back returns to the caller.

 * Pass returnTab when opening from a tab (fallback when stack history is empty, e.g. deep links).

 */

export function navigateToArticleDetail(navigation, item, options = {}) {

  const { returnTab, ...rest } = options;

  const tab =

    returnTab ||

    (typeof item === 'object' && item?.returnTab) ||
...
/** Open article detail from the root NavigationContainer ref (push notifications, deep links). */

export function navigateToArticleDetailFromRoot(navRef, articleId, returnTab = 'Notifications') {

  const id = String(articleId || '').trim();

  if (!id) return false;

  const params = buildArticleDetailParams({ id }, { returnTab });

  return pushMainStackScreenFromRoot(navRef, 'ArticleDetail', params);
