...
export function normAdminCategories(raw) {
  return (Array.isArray(raw) ? raw : [])
    .map((x) => {
      ...
      const slug = String(x?.slug || x?.id || x?.name || '').trim().toLowerCase().replace(/\s+/g, '-');
      ...
      const subs = (x?.subcategories || [])
        .map((s) => {
          ...
          const subSlug = String(s?.slug || s?.id || s?.name || '').trim().toLowerCase().replace(/\s+/g, '-');
...
/** Normalize admin connection rows. */
...
export function toAdminPayloadList(items) {
  return normAdminCategories(items);
}
